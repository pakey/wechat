...
class Encryptor
{
     ...
     * @throws EncryptionException
     */
    public function decryptComponent($postXML, $appid = null)
    {
        try {
            $array = XML::parse($postXML);
        } catch (BaseException $e) {
    ...
    /**
     * encrypt Component message.
     *
     * @return array
     * @throws EncryptionException
     */
    public function encryptComponent($xml, $appid = null)
    {
        if (is_array($xml)) {
            $xml = XML::build($xml);
        }
        ...
        $encrypt = $this->encrypt($xml, $appid);
    
        $timestamp = time();
        $nonce     = substr(md5($appid.$timestamp), 0, 10);
